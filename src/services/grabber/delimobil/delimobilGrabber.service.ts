...
import {delimobilCarsToCommonCars} from '../utils/delimobilCarsToCommonCars';
...
export class DelimobilGrabService implements IGrabberService {
    private apiService = new APIService();

    getCars(): Observable<ICommonCar[]> {
        return this.apiService.get<IDelimobilAPIResponse>(DELIMOBIL_API_URL)
            .map(resp => delimobilCarsToCommonCars(resp.features));
    }
}